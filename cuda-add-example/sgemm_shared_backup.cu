...
#define THREAD_DIM 16         // 线程块维度：16x16 = 256 线程（合法）
#define ELEM_PER_THREAD 4     // 每个线程在每个维度上处理的元素数（4x4=16）
...
__global__ void sgemm_shared_v2(const float* A, const float* B, float* C, int M, int N, int K) {
    ...
    for (int tile = 0; tile < (K + TILE_SIZE - 1) / TILE_SIZE; tile++) {
        // 协作加载 A 和 B 的子块到共享内存
        // 每个线程加载一个 float，但需要加载 4x4 个？不，由于线程块是 16x16，
        // 而 TILE_SIZE=64，每个线程需要循环加载多个元素。
        for (int i = 0; i < ELEM_PER_THREAD; i++) {
            for (int j = 0; j < ELEM_PER_THREAD; j++) {
                int a_row = row_start + i;
                int a_col = tile * TILE_SIZE + tx * ELEM_PER_THREAD + j;
                int as_row = ty * ELEM_PER_THREAD + i;
                int as_col = tx * ELEM_PER_THREAD + j;
                As[as_row][as_col] = (a_row < M && a_col < K) ? A[a_row * K + a_col] : 0.0f;
